...
- **色彩**：所有文本 WCAG AA 对比度，语义化 token，状态色限 4 类
...
**源码级组件而非包依赖。** 组件以源码形式安装到用户项目（`src/components/anchor-ui/`），用户拥有全部代码，可以自由修改。这种「复制而非引用」的分发模型确保组件不会因上游版本升级而 break，同时允许项目级定制。
