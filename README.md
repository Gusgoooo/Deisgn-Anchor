...
3. 统一视觉：优先对接 GenDesignSystem / Theme Lab；没有时建立或复用 `design-tokens.json`，用 token-bound 组件重装页面。
...
读取已有页面
  -> 提取业务逻辑和用户任务
  -> 对接 Theme Lab 或 design-tokens.json
  -> 选择 UX 策略
  -> 用统一组件体系重组页面
  -> 接回原有数据、状态、权限和事件
  -> 补齐状态、响应式、可访问性和视觉细节
...
GenDesignSystem / Theme Lab 是优先 token 源。项目已经有 `theme-lab.json` 或 `theme-lab:runtime` CSS marker 时，Skill 会消费它的 shadcn adapter 和语义变量，不会再静默创建一套并行 `design-tokens.json`。

没有 Theme Lab 时，`design-tokens.json` 是样式统一的 fallback 契约。Skill 可以创建、补全或映射 token，但不应静默覆盖用户已经定义的主题。
...
2. Tailwind 项目默认采用 shadcn 组件语义；如果有 Theme Lab，则通过它的 shadcn adapter 变量驱动组件样式。
3. 其他组件库通过 mapping 接入主题入口，例如 Ant Design、MUI、Chakra、Mantine、Radix-based 组件体系。
4. Vercel UI skills 可作为执行参考：shadcn 负责组件、v0 输出要 normalize、Geist 提供字体纪律、browser verify 负责视觉检查。
5. 组件只负责统一执行，真正的页面结构由业务任务和 UX 判断决定。
...
- `references/gendesignsystem-token-bridge.md`：GenDesignSystem / Theme Lab token 合同、runtime marker、shadcn adapter 对接规则。
- `references/functional-ux.md`：业务逻辑识别、交互诊断和重组策略。
- `references/component-isolation.md`：AI 辅助前端重构时的组件边界、状态归属和防过度组件化规则。
- `references/vercel-ui-skill-routes.md`：Vercel UI skills 的可借鉴路径：shadcn、v0、Geist、AI Elements、React review、browser verify。
